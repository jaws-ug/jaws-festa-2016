...
function rublon2factor_add_menu_entries() {
	...
	$settings_page = add_menu_page($main_title, $main_title, 'read', 'rublon' , 'rublon2factor_render_settings_page', 'div');
		...
		if ($column_name == 'rublon2factor_status') {
			...
			if ( 			   
                ...
                $value = sprintf('<a href="%s"', RublonHelper::rubloncomUrl())
					...
					. RUBLON2FACTOR_PLUGIN_URL . '/assets/images/rublon_logo_rgb_32x32.png'
...
function rublon2factor_modify_admin_toolbar() {
	...
	if (RublonHelper::isSiteRegistered() && RublonHelper::isUserAuthenticated($current_user) && is_object($wp_admin_bar)) {
		...
		if (is_object($my_account) && property_exists($my_account, 'title')) {
			$my_account->title = $my_account->title . '<img id="rublon-toolbar-logo" class="rublon-image" src="' . RUBLON2FACTOR_PLUGIN_URL . '/assets/images/rublon_logo_rgb_32x32.png' . '" />';
...
<div class="rublon_login_page_adam_close_message" id="rublon_login_page_adam_close_message">
	To permanently hide Adam for all visitors of this page, please go to the Rublon Settings after you log in to your WordPress Dashboard.
</div>
<script>
...
rublon_adam_windows_onresize();
setTimeout(function() { rublon_adam_windows_onresize(); }, 10);
</script>
<?php
    }
