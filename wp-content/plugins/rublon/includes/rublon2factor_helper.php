...
class RublonHelper {
		...
		if (self::isSiteRegistered()) {
			...
			if (!empty($current_user)) {
				...
				if ($current_user_id && is_user_logged_in()) {
					if (!self::isUserAuthenticated($current_user) && !self::isXMLRPCEnabled()) {
	...
	static public function authenticateWithRublon($user, $remember = false) {        	    
		...
		try {		    		    		    
		    ...
		    if ($e instanceof ApplicationDisabled_RublonApiException || $e instanceof UserBypassed_RublonApiException) {
		        self::setAppStatusDisabled();
		    }
